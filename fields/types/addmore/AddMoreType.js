...
function addmore(list, path, options) {
	...
	addmore.super_.call(this, list, path, options);
}
...
addmore.prototype.addToSchema = function() {
	...
	schema.add({
		...
		videoThumbnail: getFieldDef(String, 'videoThumbnail'),
...
addmore.prototype.format = function(item, values, delimiter) {
...
addmore.prototype.isModified = function(item) {
...
addmore.prototype.validateInput = function(data, required, item) {
...
addmore.prototype.getRequestHandler = function(item, req, paths, callback) {
...
addmore.prototype.handleRequest = function(item, req, paths, callback) {
...
addmore.prototype.googleLookup = function(item, region, update, callback) {
	...
	doGoogleGeocodeRequest(address, region || keystone.get('default region'), function(err, geocode){
		...
		_.each(result.address_components, function(val){
			if ( _.indexOf(val.types, 'street_number') >= 0 ) {
				...
				location.videoId.push(val.long_name);
			}
			if ( _.indexOf(val.types, 'route') >= 0 ) {
				location.videoId = location.videoId || [];
				location.videoId.push(val.short_name);
			}
...
addmore.prototype.kmFrom = function(item, point) {
...
addmore.prototype.milesFrom = function(item, point) {
...
exports = module.exports = addmore;
